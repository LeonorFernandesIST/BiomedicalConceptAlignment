...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.pe = PositionalEncoding (d_model=CHAR_SIZE,dropout=DROPOUT_P)
    ...
    def forward(self, s, lengths):
        ...
        hidden_1 = self.init_hidden(s_pos.size()[0])
        hidden_2 = self.init_hidden(s_pos.size()[0])

        # Dynamic sequence length
        s = torch.nn.utils.rnn.pack_padded_sequence(s_pos, lengths, batch_first=True, enforce_sorted=False)
...
class RSPoolModelEmb(pl.LightningModule):
    def __init__(self, name, train_dataset, test_dataset):
        super(RSPoolModelEmb, self).__init__()
